...
import { Plus, Edit, Trash2, Home, X, AlertTriangle, Eye, ImageIcon } from 'lucide-react';
  ...
  return (
    <div className="flex flex-col gap-8 font-sans relative">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {services.length === 0 ? (
           <p className="text-gray-400 col-span-full">Belum ada layanan yang ditambahkan.</p>
        ) : (
          services.map((service, index) => (
            ...
            >
              <div className="flex items-center gap-4">
                {service.imageUrl ? (
                  <div className="w-16 h-16 rounded-sm overflow-hidden relative shrink-0">
                    <Image src={`${IMAGE_BASE_URL}${service.imageUrl}`} alt={service.name} fill className="object-cover" unoptimized />
                  </div>
                ) : (
                  <div className="w-16 h-16 rounded-sm bg-[#DDF247]/10 flex items-center justify-center text-[#DDF247] shrink-0">
                    <Home className="w-6 h-6" />
                  </div>
                )}
                
                <div className="flex-1 overflow-hidden">
                  <h3 className="text-lg font-medium text-white mb-1 truncate">{service.name}</h3>
                  <span className={`text-xs px-2 py-0.5 font-medium rounded-sm inline-block ${
              ...
              <p className="text-sm text-gray-400 leading-relaxed line-clamp-3 h-[60px]">{service.description}</p>

              <div className="flex items-center justify-end mt-2 pt-4 border-t border-white/5 gap-2">
                  <button 
        ...
        {previewService && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm">
            <motion.div 
              ...
              className="bg-[#181A1F] border border-white/10 w-full max-w-lg rounded-sm overflow-hidden flex flex-col shadow-2xl max-h-[90vh] overflow-y-auto"
            >
              ...
              <div className="p-6 flex flex-col gap-6 text-center items-center">
                <div className="w-full h-48 bg-[#121418] rounded-sm border border-white/5 flex items-center justify-center text-gray-500 relative overflow-hidden">
                   {previewService.imageUrl ? (
                     <Image src={`${IMAGE_BASE_URL}${previewService.imageUrl}`} alt={previewService.name} fill className="object-cover" unoptimized />
                   ) : (
                ...
                <div className="w-full text-left bg-[#121418] p-4 rounded-sm border border-white/5">
                  <p className="text-gray-500 mb-2 text-sm">Deskripsi Layanan</p>
                  <p className="text-gray-300 text-sm leading-relaxed whitespace-pre-wrap">{previewService.description}</p>
                </div>
              </div>
            ...
            >
              <div className="p-8 flex flex-col items-center text-center gap-4">
                ...
                <p className="text-gray-400 text-sm leading-relaxed">
                  Anda yakin ingin menghapus layanan <span className="text-white font-medium">"{serviceToDelete.name}"</span>? Tindakan ini tidak dapat dibatalkan.
                </p>
              </div>
